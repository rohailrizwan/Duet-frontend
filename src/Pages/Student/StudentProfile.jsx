    ...
    <Box
      sx={{
        ...
        background: `linear-gradient(45deg, ${
          color || Colors?.PrimaryBlue || "#3b5998"
        }, #5c80bc)`,
        borderRadius: "50%",
        width: 40,
        height: 40,
...
// ChipSection component with gradient chips
const ChipSection = ({ title, data }) => (
  <>
    ...
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, pl: 2 }}>
      {data?.length > 0 ? (
        data.map((item, idx) => (
          <Chip
            key={idx}
            label={item}
            sx={{
              background: "linear-gradient(135deg, #3b5998, #5c80bc)",
              color: "#fff",
              fontWeight: "600",
              px: 2,
              py: 0.5,
              borderRadius: 2,
              boxShadow: "0 4px 8px rgba(59, 89, 152, 0.4)",
              cursor: "pointer",
    ...
    return (
      <Box
        sx={{
          ...
          justifyContent: "center",
          alignItems: "center",
          background: "linear-gradient(120deg, #e6ecf5, #d1deff)",
        }}
      >
        <CircularProgress sx={{ color: Colors?.PrimaryBlue || "#3b5998" }} />
      </Box>
    );
  }
            ...
            <Grid container spacing={4} >
                    ...
                    <Chip
                      ...
                      sx={{
                        ...
                        "&:hover": {
                          transform: "scale(1.1)",
                          background:
                            "linear-gradient(45deg, #2a4373, #4a6ea3)",
              ...
              <Grid item xs={12}>
                ...
                >
                  <CardContent>
                    ...
                    {/* Social Links */}
                    <Box
                      sx={{ mt: 4, display: "flex", gap: 3, flexWrap: "wrap" }}
                    >
                      {data?.linkedInUrl && (
                        <Link
                          ...
                          sx={{
                            ...
                            textDecoration: "none",
                            background:
                              "linear-gradient(45deg, #3b5998, #5c80bc)",
                            px: 2,
                            py: 1,
                            borderRadius: 2,
                            transition: "all 0.3s ease-in-out",
                      ...
                      {data?.gitHubUrl && (
                        <Link
                          ...
                          sx={{
                            ...
                            textDecoration: "none",
                            background:
                              "linear-gradient(45deg, #1f2a44, #374151)",
                            px: 2,
                            py: 1,
                            borderRadius: 2,
                            transition: "all 0.3s ease-in-out",
                            "&:hover": {
                              transform: "scale(1.05)",
                              background:
                                "linear-gradient(45deg, #1f2a44, #4b5563)",
                            },
                          }}
                        >
                          <GitHubIcon />
              ...
              <Grid item xs={12}>
                <Divider
                  sx={{
                    my: 4,
                    background: "linear-gradient(90deg, #3b5998, #5c80bc)",
                  }}
                />
              </Grid>
              <Grid item xs={12}>
              ...
              <Grid item xs={12}>
                ...
                <Box display="flex" alignItems="center" sx={{ mb: 2, pl: 2 }}>
                  ...
                  <Typography variant="body1">
                    Dawood University - Enroll Number:{" "}
                    {data?.academicDetails?.enrollNumber || "N/A"}, Semester:{" "}
                    {data?.academicDetails?.semester || "N/A"}
                  </Typography>
                </Box>
                <Box display="flex" alignItems="center" sx={{ mb: 2, pl: 2 }}>
                  <SchoolIcon sx={{ mr: 2, color: "#3b5998" }} />
                  <Typography variant="body1">
                    {data?.intermediateDetails?.collegeName || "N/A"} - Passing
                    year: {data?.intermediateDetails?.passingYear || "N/A"} -
                    Grade: {data?.intermediateDetails?.grade || "N/A"}
                  </Typography>
                </Box>
                <Box display="flex" alignItems="center" sx={{ mb: 2, pl: 2 }}>
                  <SchoolIcon sx={{ mr: 2, color: "#3b5998" }} />
                  <Typography variant="body1">
                    {data?.matriculationDetails?.schoolName || "N/A"} - Passing
                    year: {data?.matriculationDetails?.passingYear || "N/A"} -
                    Grade: {data?.matriculationDetails?.grade || "N/A"}
                  </Typography>
                </Box>
              </Grid>
              <Grid item xs={12}>
                    ...
                    {data.workExperience.map((exp, index) => (
                      <Card
                        ...
                        sx={{
                          ...
                          borderRadius: 3,
                          background:
                            "linear-gradient(45deg, #f5f8ff, #e6ecf5)",
                          transition: "all 0.3s ease-in-out",
                          "&:hover": {
                      ...
                      >
                        <Box display="flex" alignItems="center" gap={2}>
                          <WorkIcon
                            sx={{ color: Colors?.PrimaryBlue || "#3b5998" }}
                          />
                          <Box>
                            <Typography
                              variant="subtitle1"
